...
- `Dockerfile` (multi-stage, non-root runtime) and `docker-compose.yml` (`api`, `claims-system`, `jaeger` services, shared `casefile-data` volume).
- `env.example` — configuration template covering the model, every ceiling, the approval secret, storage and tracing settings.
- `src/casefile` package skeleton: `config.py` (`Settings`, all ceilings), `api/app.py` (FastAPI factory with `/healthz`).
